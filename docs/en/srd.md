...
- [System Reference Document](#system-reference-document)
  ...
  - [Character Creation](#character-creation)
    - [Name, Background, Assets](#name-background-assets)
...
**Cyber** was written with the following design philosophies in mind:
...
Choose a name for your character and their [background](./background.md), which informs about knowledge and potential skills from the character's life experience.
...
- **Body (BOD)** - overall physical qualities: build, endurance, agility, dexterity, reflexes
- **Psyche (PSY)** - a set of mental and social qualities: intelligence, knowledge, willpower, empathy, contacts
- **Interface (INT)** - characteristics related to the use of technology: ability to operate in the Infosphere, resistance to cyber attack, level of integration with cyber modifications.
...
> **Example:**
> Miko rolls for her character’s BOD, resulting in a 2, a 4, and a 6, totaling 12. The next two ability rolls result in a 9 for PSY and a 13 for INT. She decides to swap the 12 and the 9, for a character with 9 BOD, 12 PSY and 13 INT.
...
- 2 slots  - **hands**, i.e. whatever the hero is currently holding - weapons, tools, etc.
- 2 slots - **body** - Any place such as the back, belt, chest. They can be used to attach another container such as a backpack or bag.
...
Bags, backpacks, trolleys can increase the number of inventory slots, the same also applies to cybermodifications. Whereby, items representing containers attached to the character's body or requiring the use of the hand (carried, pulled) **take up space** in the inventory.
...
- **Body (BOD)** - in risky situations requiring physical strength, agility or stamina.
- **Psyche (PSY)** - for troubles that can be avoided through intelligence, knowledge, willpower or social interaction.
- **Interface (INT)** - in avoiding Infosphere threats, problems with cyber modifications or device control.
...
Anyone deprived for more than a day adds **Fatigue** to their inventory, one for each day. Each Fatigue occupies one slot and lasts until they are able to recuperate (such as a full night’s rest in a safe spot). After a rest, the character recovers *all* taken slots.

Slots blocked by Fatigue cannot be used to carry equipment, if a player has to mark Fatigue on a slot already occupied by an item, that item must be put back or discarded.
...
Sometimes the facilitator would like to point out that it is easier or more difficult for a character to avoid risks or negative consequences of actions (other than combat) due to <u>external circumstances</u>, independent of the character's abilities.
...
The referee will telegraph the most likely actions taken by NPCs. At the start of combat, each PC must make a save to act before their opponents. For a physical combat that should be a BOD test, for the Infosphere - PSY test.

> **Example:** *Bea has accidentally stumbled upon an area where a gang of smugglers is prowling. In order to act before them, she has to take a BOD test. She fails, so the smugglers attack first.*

#### Attacking & Damage

The attacker rolls the **damage dice** for their weapon. For physical damage (inflicted in the BOD) subtracts the **armour** of the target, then deals the remaining damage to the **opponent’s HP**.

**Unarmed** attacks always deal **1d4** damage.

If the amount of the damage exceeds the opponent's current HP, the remaining points **decrease the corresponding ability** of the character (BOD for combat in the physical world, INT for clash in the Infosphere or EMP weapons).

- Reducing any of the character's abilities to zero means:
  - BOD - physical death of the character
  ...
  - INT - blocked  access to the Infosphere until at least one point of this ability has been regained, but not less than 24 hours, and failure of all cyber modifications
...
Damage that reduces a target’s HP below zero decreases a target’s ability (BOD or INT) by the amount remaining.
They must then make a BOD/INT save (with the ability value after reduction) to avoid **critical damage**.
...
Any PC that suffers BOD critical damage cannot do anything but crawl weakly, grasping for life. If given aid and rest, they will stabilize. If left untreated, they die within the hour.

Any PC that suffers INT critical damage, becomes immobilised in the Infosphere (cannot move between nodes) or all its cyber modifications cease to function if it operates in the physical world.

The **Black ICE** and **EMP weapons** have an additional critical damage effect: *"if the target has a neuroprocessor, inflict as many points of damage to the target's PSY as have been inflicted in INT this turn "*.
...
Running away from a dire situation always requires a successful save (BOD for physical world, INT for the Infosphere), as well as a safe destination to run to.
...
Lost BOD or PSY points can usually be restored by a week's rest supported by medical systems or an appropriate specialist.

Lost INT points require a visit to a cybertechnician or self-repair if possible.

### Economy

The global currency in the game is **credit** denoted by **ℂ**.

Characters start the game with a pool of **1k4 * 1000** credits.
...
All risky situations in the Infosphere are dealt with using the **Interface** ability test. The same is true for damage that exceeds the character's HP level - these cause the Interface ability to be reduced instead of the Body ability.
...
All other activities such as hacking, security, access to proprietary information require appropriate cyber modifications.

**Infosphere actions** are resolved similarly to the physical world, except that the ability taken into account for saves is Interface.
Similarly, when a character's HP is reduced to zero, the ability that is decreased by attacks is Interface instead of Body.

**Lock**: Reducing a character's INT to zero means that they cannot use the Infosphere until they have regained at least one point of this ability but no less than 24 hours.

**Hacking**: Network nodes can have an HP level and an INT level specified. To access such a node (without the owner's permission), the player must reduce its HP to zero. To do this, attacks are required using appropriate cyber modifications. Once the HP is removed, the player takes control of the node for **as many rounds** as his **INT level**.

**Data destruction**: After reducing the node's HP to zero, the player can still perform the attack but this time will reduce the node's INT. Reducing a node's **INT to zero** means irrevocably **destroying the data or program** (unless its owner had a backup). Reducing a node's INT to values other than zero has no effect on the game.

**Firewall**: Player characters can also actively protect Infosphere nodes. To do so, they must have the appropriate cyber modification and pass an INT test. Success means that this node is completely immune to hacking in the next turn.

**Intrusion Countermeasures Electronics (ICE)**: Infosphere nodes can have automatic defense systems. This means they have an additional **ICE** factor expressed in **damage dice** (d4 to d12). Each round of hacking they make an attack on a character, reducing hacker's INT. Hackers recognize the 'white' and 'black' ICE.

**Black ICE** (officially illegal), has an additional feature:

- *Critical Damage*: inflict in the PSY of the target, the same number of points of damage as you have inflicted in the INT this turn.

### Cybermodifications

Each character may install cyber modifications for himself/herself in an amount not exceeding the **Interface ability level divided by 3** (rounding up).

> Example: *John Kowalsky's* has Interface ability equal to 10. This means that he can install 4 cyber modifications.

A character whose **INT is zero** cannot use cyber modifications - they stop working until the character has regained at least one point of Interface.

For more information on the creation of cyber modifications and their pricing, see [Appendix](./cybermod.md).
...
Hirelings do not have their own cyber modifications but the player can equip them at his own expense if he wishes.
...
**Name**  
X HP, X Armor, X BOD, X PSY, X INT, Weapon (dX, special items, qualities)
